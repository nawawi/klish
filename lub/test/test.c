...
static lub_test_status_t checkStatus(lub_test_status_t value)
{
...
/* NAME:    unitTestLog
   ...
   RETURN:  none
*/
static void unitTestLog(lub_test_verbosity_t level, const char *format, ...)
{
	va_list args;
	...
	if (level <= verbosity) {
		if (LOGGING_TO_FILE) {
			if (NULL != logp) {
				...
				fprintf(stderr,
					"ERROR: Trying to log to file, but no logfile is open!\n");
			}
		}
...
/* NAME:    lub_test_begin
   ...
   RETURN:  none
*/
void lub_test_begin(const char *name, ...)
{
	va_list args;
...
void lub_test_end(void)
{
	...
	if ((termMode == ContinueOnFail) || (unitTestStatus == LUB_TEST_PASS)) {
		...
		unitTestLog(LUB_TEST_TERSE,
			    "END: Test '%s': STOPPED AT FIRST FAILURE.\n",
			    unitTestName);
	}
	if (LOGGING_TO_FILE) {
		fclose(logp);
...
static void printUsage(void)
{
	printf("Usage:\n");
	printf
	    ("-terse, -normal, -verbose    : set lub_test_verbosity_t level (default: normal)\n");
	printf
	    ("-stoponfail, -continueonfail : set behavior upon failure (default: continue)\n");
	printf
	    ("-logfile [FILENAME]          : log to FILENAME (default: 'test.log')\n");
	printf("-nologfile                   : disable logging to file \n");
	printf
	    ("-stdout, -nostdout           : enable/disable logging to STDOUT\n");
...
void lub_test_parse_command_line(int argc, const char *const *argv)
{
	...
	while (--argc > 0) {
		...
		/* lub_test_verbosity_t Levels */
			...
			verbosity = LUB_TEST_TERSE;
			verbset++;
		} else if (strstr(argv[argc], "-normal") != NULL) {
			verbosity = LUB_TEST_NORMAL;
			verbset++;
		} else if (strstr(argv[argc], "-verbose") != NULL) {
			verbosity = LUB_TEST_VERBOSE;
			verbset++;
		}
		/* Failure behavior */
		...
		/* Log file options */
			...
			if (strstr(argv[argc + 1], "-") != argv[argc + 1]) {
				/* Yes, got a filename */
				logFile =
				    (char *)malloc(strlen(argv[argc + 1]) + 1);

				if (NULL == logFile) {
					status = BOOL_FALSE;
					fprintf(stderr,
						"unitTestParseCL: ERROR: Memory allocation problem.\n");
				} else {
		...
		/* Unhandled arguments */
			...
			if (strstr(argv[argc - 1], "-logfile") == NULL) {
				...
				fprintf(stderr,
					"Unrecognized argument: '%s', ignoring it...\n",
					argv[argc]);
			}
		}
	}
	...
	if (logDest == 0) {
		fprintf(stderr,
			"WARNING: No logging is enabled to either stdout or a logfile; expect no output.\n");
	}
	/* Make sure there were no conflicting options */
	if (verbset > 1) {
		fprintf(stderr,
			"ERROR: conflicting lub_test_verbosity_t options specified.\n");
		fprintf(stderr,
			"       Specify only ONE of -terse, -normal, -verbose\n");
		status = BOOL_FALSE;
	}
	if (failset > 1) {
		fprintf(stderr,
			"ERROR: conflicting Failure Mode options specified.\n");
		fprintf(stderr,
			"       Specify only ONE of -stoponfail, -continueonfail\n");
		status = BOOL_FALSE;
	}
	if (logfset > 1) {
		fprintf(stderr,
			"ERROR: conflicting Logfile options specified.\n");
		fprintf(stderr,
			"       Specify only ONE of -logfile, -nologfile\n");
		status = BOOL_FALSE;
	}
	if (stdset > 1) {
		fprintf(stderr,
			"ERROR: conflicting Stdout options specified.\n");
		fprintf(stderr,
			"       Specify only ONE of -stdout, -nostdout\n");
		status = BOOL_FALSE;
	}
...
/* NAME:    lub_test_seq_begin
   ...
   RETURN:  none
*/
void lub_test_seq_begin(int num, const char *seqName, ...)
{
	va_list args;
	...
	if (num == seqNum) {
		seqNum++;
		unitTestLog(LUB_TEST_TERSE,
			    "seq_start: duplicate sequence number.  Using next available sequence number (%d).\n",
			    seqNum);
	} else {
		seqNum = num;
	}
	...
	lub_test_seq_log(LUB_TEST_NORMAL, "*** Sequence '%s' begins ***",
			 seqDescr);
}
...
/* NAME:    lub_test_seq_log
   ...
   RETURN:  none
*/
void lub_test_seq_log(lub_test_verbosity_t level, const char *format, ...)
{
	va_list args;
	char string[160];
...
*/
lub_test_status_t lub_test_check(bool_t expr, const char *testName, ...)
{
	va_list args;
	char testDescr[80];
		     ...
		     const char *testName, ...)
{
	...
	/* evaluate expression */
	testStatus = ((actual >= min)
		      && (actual <= max)) ? LUB_TEST_PASS : LUB_TEST_FAIL;
...
/* NAME:    lub_test_log
   ...
   RETURN:  none
*/
void lub_test_log(lub_test_verbosity_t level, const char *format, ...)
{
	va_list args;
	char string[160];
